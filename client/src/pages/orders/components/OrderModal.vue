      ...
      <form @submit.prevent="saveOrder" class="space-y-4">
        ...
        <div v-if="selectedProduct">
          ...
          <p class="text-gray-700">
            {{ products.find((p) => p.id === selectedProduct).name }} Price: €{{
          ...
          <label class="text-gray-700"
            ...
            (Stock: {{ selectedProductStock - quantityToAdd }})</label
          >
          <input
            type="number"
    ...
    addProductToOrder() {
      if (this.selectedProduct && this.quantityToAdd > 0) {
        ...
        if (existingProductIndex !== -1) {
          this.order.products[existingProductIndex].quantity += parseInt(
            this.quantityToAdd
          );
          this.order.products[existingProductIndex].totalPrice =
            this.order.products[existingProductIndex].quantity *
            productToAdd.price;
        } else {
        ...
        this.order.totalAmount = this.order.products
